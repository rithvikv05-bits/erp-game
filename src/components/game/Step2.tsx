...
const REQUIRED_COURSES = ["CS F404", "CS F499", "PHIL F202", "MUS F100"];
...
export default function Step2({ onCourseSelect, onBack, currentCourseIndex, selectedCourses }: Step2Props) {
  ...
  const courseType = currentCourseIndex < 2 ? "CS Discipline Elective" : "Open Elective";
    ...
    <Card className="w-full">
      <CardHeader>
        <div className="flex items-center gap-4">
          ...
          <CardTitle className="font-headline text-2xl">Step 2: Find Course ({currentCourseIndex + 1}/4)</CardTitle>
        </div>
        <CardDescription>
          Use the state-of-the-art search function to find your required <span className="font-bold text-primary">{courseType}</span>: <span className="font-bold text-primary">{requiredCourseId}</span>.
        </CardDescription>
      </CardHeader>
