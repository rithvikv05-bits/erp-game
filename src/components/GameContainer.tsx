...
export default function GameContainer() {
  ...
  const handleTutorialSelect = (course: Course) => {
    ...
    if (currentCourseIndex < 3) {
        setCurrentCourseIndex(prev => prev + 1); // Move to next course
        setStep(2); // Go back to course search for the next course
    } else {
        setStep(5); // All 4 courses selected, go to cart
    }
  }
  ...
  const handleCompletion = async () => {
    ...
    await addScore({ name: user.name, time });
    setStep((prev) => prev + 1);
  };
